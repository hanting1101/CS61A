...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    typed_len = len(typed) / 5
    return typed_len * (60/elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    str = word_list[0]
    for x in word_list:
        var = diff_function(typed_word, x, limit)
        if var < MIN:
            MIN = var
            str = x
    if MIN > limit:
...
def report_progress(typed, source, user_id, upload):
    ...
    def fn(typed, source):
        if (not typed) or (not source):
            return 0
        if typed[0] == source[0]:
            return 1 + fn(typed[1:], source[1:])
        return 0
    Progress = fn(typed, source) / len(source)
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        mini = get_time(times, 0, i)
        index = 0
        for j in player_indices:
            current = get_time(times, j, i)
            if current < mini:
                mini = current
                index = j
